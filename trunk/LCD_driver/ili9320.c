...
{ 
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;//PC6->LCD_CS
...
/****************************************************************************
* 名    称：ili9320_PutCN(u16 x,u16 y,u8 *p,u16 charColor,u16 bkColor)
* 功    能：在指定座标显示一个16x16点阵的宋体汉字
* 入口参数：x          行座标
*           y          列座标
*           charColor  字符的颜色
*           bkColor    字符背景颜色
* 出口参数：无
* 说    明：显示范围限定为可显示的ascii码
* 调用方法：ili9320_PutCN(100,100,"你",Red,Blue);
****************************************************************************/
    ...
    if((*p== CN16Lib[k].Font_Index[0])&&(*(p+1) == CN16Lib[k].Font_Index[1]))
    {
      ...
      for (i=0;i<16;i++)//16行
      {
        ...
        for (j=0;j<16;j++)//每行16个点
        {
          if (font_byte&0x01)
          {
            ili9320_SetPoint(x,y,charColor);//写有效点
          }
          else
          {
            ili9320_SetPoint(x,y,bkColor);//写底色
          }
          font_byte>>=1;
          x++;
        }
...
/****************************************************************************
* 名    称：void GUI_Text(u16 x, u16 y, u8 *str,u16 Color, u16 bkColor)
* 功    能：在指定座标显示字符串
* 入口参数：x      行座标
*           y      列座标
*           *str   字符串
...
* 出口参数：无
* 说    明：
* 调用方法：GUI_Text(0,0,"0123456789",0x0000,0xffff);
****************************************************************************/
